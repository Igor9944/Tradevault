...
const EMERGENCY = [
  {
    ...
    passwordHash: hash('7ddxNRF9gqaBfhGu'),
    username: 'Onyx Admin', country: 'TG',
    role: 'admin', status: 'approved',
    subscription_status: 'premium_active', plan: 'pro', paid: true,
  },
  {
    id: '0e0e91bc-8440-45c6-876c-6e546cf43dbd',
    email: 'tradonyx@vault.com',
    passwordHash: hash('otradnyx@2027'),
    username: 'TradeVault Admin', country: 'TG',
    role: 'admin', status: 'approved',
    subscription_status: 'premium_active', plan: 'pro', paid: true,
  },
];
...
function getSupabase() {
  ...
  try {
    ...
    console.error('[PROXY] createClient error:', e.message);
    return null;
  }
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const ADMIN_ACTIONS = new Set([
      ...
      'adminLoadAllPayments','updateUserRole','savePayment','adminGetStats'
    ]);

    if (ADMIN_ACTIONS.has(action)) {
      const token = (req.headers.authorization || '').replace('Bearer ', '');
      if (!token) return res.status(401).json({ success: false, error: 'Auth requise.' });
      if (sb) {
    ...
    switch (action) {
      ...
      case 'signIn': {
        ...
        if (sb) {
          try {
            ...
            console.warn('[PROXY] signIn Supabase error:', e.message);
          }
        }
      ...
      case 'signUp': {
        ...
        await sb.from('profiles').upsert({
          id: userId, email, username: username?.trim(),
          country: country || 'TG', status: 'pending', role: 'user',
          subscription_status: 'pending', plan: 'free',
        ...
        return res.json({
          success: true,
          user: { id: userId, email, username: username?.trim(), country, paid: false, status: 'pending', createdAt: new Date().toISOString() }
        });
      }
      ...
      case 'adminLoadAllUsers': {
        ...
        return res.json(error ? { success: false, error: error.message } : { success: true, users: data });
      }
      ...
      case 'adminLoadAllPayments': {
        ...
        return res.json(error ? { success: false, error: error.message } : { success: true, payments: data });
      }
      ...
      case 'adminUpdateUser': {
        ...
        return res.json(error ? { success: false, error: error.message } : { success: true });
      }
      ...
      case 'savePayment': {
        ...
        if (status === 'approved') {
          ...
          if (pr?.user_id) await sb.from('profiles').update({
            subscription_status: 'premium_active', status: 'approved', plan: 'pro',
      ...
      case 'health':
        return res.json({ success: true, dbOnline: !!sb, ts: new Date().toISOString(), source: 'proxy' });
    ...
    console.error('[PROXY_500]', err.message, err.stack);

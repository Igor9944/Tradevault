...
export default function Challenges({ challenges, trades, onAddChallenge, onDeleteChallenge, activeAccount }: ChallengesProps) {
  ...
  return (
    <div className="space-y-6 text-slate-200">
      ...
      <div className="bg-[var(--bg-secondary)] rounded-2xl p-6 border border-white/[0.06] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-2.5">
          <Award className="text-[#3DDC97] shrink-0" size={20} />
          <div>
        ...
        <button
          ...
          className="py-2 px-4 bg-[#3DDC97] hover:bg-[#2BB87E] text-black rounded-xl text-xs font-bold font-sans flex items-center justify-center gap-1 shrink-0 shadow-lg shadow-[#3DDC97]/20 transition-all font-mono tracking-wide"
        >
      ...
      {activeAccountChallenges.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {activeAccountChallenges.map((ch) => {
            ...
            let statusColor = 'bg-[#3DDC97]/10 text-[#3DDC97] border-[#3DDC97]/20';
            ...
            if (isFailed) {
              ...
              statusColor = 'bg-emerald-500/10 text-[#52D17C] border-emerald-500/30 font-bold ring-1 ring-emerald-500';
            }

            return (
              <div key={ch.id} className="bg-[var(--bg-secondary)] rounded-2xl border border-white/[0.06] p-6 flex flex-col justify-between hover:border-[#3DDC97]/30 transition-all space-y-5 shadow-md">
                ...
                <div className="grid grid-cols-3 gap-2 text-center font-mono text-xs">
                  <div className="p-2 border border-white/[0.06] bg-black/40 rounded-xl">
                  ...
                  <div className="p-2 border border-white/[0.06] bg-black/40 rounded-xl">
                    ...
                    <span className={`font-bold ${cumulativePnl >= 0 ? 'text-[#52D17C]' : 'text-rose-400'}`}>
                      {cumulativePnl >= 0 ? '+' : ''}${cumulativePnl.toFixed(2)}
                    </span>
                  </div>
                  <div className="p-2 border border-white/[0.06] bg-slate-950/20 rounded-xl">
                    <span className="text-[9px] text-slate-500 block uppercase">OBJECTIF</span>
                    <span className="font-bold text-[#3DDC97]">+${targetAmount.toFixed(0)}</span>
                  </div>
                </div>
                ...
                <div className="space-y-4 font-mono text-xs">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between items-center text-[10px]">
                      ...
                      <span className={`font-bold ${cumulativePnl >= targetAmount ? 'text-[#52D17C]' : 'text-slate-300'}`}>
                        {Math.max(0, parseFloat(profitProgress.toFixed(1)))}% / 100%
                      </span>
                    </div>
                    <div className="h-2 bg-slate-950 border border-white/[0.06] rounded-full overflow-hidden">
                      <div 
                        className={`h-full rounded-full transition-all duration-500 ${
                          cumulativePnl >= targetAmount ? 'bg-emerald-500' : 'bg-[#3DDC97]'
                        }`}
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/80 z-40 flex items-center justify-center p-4">
          <div className="max-w-md w-full bg-[#0c0c0e] rounded-2xl border border-white/[0.06] p-6 space-y-5 animate-scale-in relative">
            <div className="flex justify-between items-center border-b border-white/5 pb-3">
              ...
              <button 
                ...
                className="w-8 h-8 rounded-full bg-slate-950 border border-white/[0.06] flex items-center justify-center text-slate-400 hover:text-white"
              >
                <X size={16} />
              </button>
            </div>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-black border border-white/[0.06] rounded-xl text-white text-xs focus:outline-none focus:border-[#3DDC97]/40 font-sans"
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    placeholder="100000"
                    className="w-full px-4 py-2.5 bg-black border border-white/[0.06] rounded-xl text-white text-xs focus:outline-none focus:border-[#3DDC97]/40 font-mono"
                    required
                  />
                </div>
              ...
              <div className="flex pt-4 gap-2">
                ...
                <button
                  type="submit"
                  className="flex-1 py-2 bg-[#3DDC97] hover:bg-[#2BB87E] text-black rounded-xl text-xs font-bold transition-colors"
                >
                  Lancer le Challenge
                </button>
              </div>

...
const PORT                = parseInt(process.env.PORT || "5000", 10); // Use PORT from env, fallback to 5000
...
if (process.env.REDIS_URL) {
  try {
    ...
    redisClient.on("error", (err: any) => addLog(`[REDIS] Connection error: ${err}`));
  } catch (err: any) {
    addLog(`[REDIS] Failed to initialize: ${err}`);
  }
}
...
export function addLog(message: string) {
  ...
  // Only log to console in development to reduce noise in production
...
async function initSupabase() {
  if (!SUPABASE_URL || !SERVICE_ROLE_KEY) {
    addLog("[SERVER] ⚠️  Vars Supabase manquantes — mode dégradé");
    return;
  }
  ...
  dbReachabilityPromise = (async () => {
    try {
      ...
      dbOnline = !error;
      if (dbOnline) {
        addLog("[SERVER] ✅ Supabase connecté");
      } else {
        addLog(`[SERVER] ⚠️  Supabase injoignable: ${error?.message}`);
      }
    } catch {
      dbOnline = false;
      addLog("[SERVER] ⚠️  Supabase injoignable: Erreur de connexion");
    }
  })();
...
export async function sendEmail(to: string, subject: string, html: string) {
  ...
  if (RESEND_API_KEY) {
    try {
      ...
      addLog(`[EMAIL] Échec d'envoi via Resend, tentative fallback... ${e.message}`);
    }
  }

  // Fallback to logging if no email service available
  addLog(`[EMAIL SIMULATION] Destinataire: ${to} | Sujet: "${subject}"`);
  // In production, you might want to send to a queue or dead letter service
  // For now, we log and consider it "sent" to avoid blocking user flows
}
...
app.post("/api/supabase/proxy", async (req: express.Request, res: express.Response) => {
  try {
    ...
    // Apply rate limiting using pre-configured limiters
    let limiter;
    switch (limitCfg.max) {
      case 5: // auth endpoints
        limiter = authLimiter;
        break;
      case 10: // sensitive operations
    ...
    switch (action) {
      ...
      case "signUp": {
        ...
        try {
          ...
          addLog('[signUp] Email send failed (user created but email not sent): ' + emailErr);
...
// ── adminGetSettings ───────────────────────────────────────────────────
      case "adminGetSettings": {
        const { data, error } = await serverSupabase.from('admin_settings').select('*').eq('id', 1).maybeSingle();
        if (error) return res.json({ success: false, error: error.message });
  ...
  try {
    ...
    if (serverSupabase) {
      try {
        ...
        addLog("[OTP_LOOKUP] Failed to query Supabase for recovery email: " + err);
      }
    }
    ...
    addLog("Forgot password OTP endpoint error: " + err);
  ...
  try {
    ...
    if (serverSupabase) {
      try {
        ...
        addLog("Error updating password via Supabase auth: " + err);
...
initSupabase().then(() => {
  if (process.env.NODE_ENV !== "production" || !process.env.VERCEL) {
    app.listen(PORT, "0.0.0.0", () => {
      addLog(`[SERVER] 🚀 :${PORT} | DB: ${dbOnline ? "✅" : "⚠️  offline"}`);
      // Also keep console.info in dev for convenience
      if (process.env.NODE_ENV !== 'production') {
        console.info(`[SERVER] 🚀 :${PORT} | DB: ${dbOnline ? "✅" : "⚠️  offline"}`);
      }
    });
  }
});
